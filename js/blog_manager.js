...
var BlogManager = (function() {
    ...
    function initHome() {
        ...
        $('#cat-link a').click(function() {
            ...
            shareURL('list', newSection, '/' + newSection);
    ...
    function createListElement(value, index) {
        ...
        $(listElem).appendTo('#list .list').children('a').click(function() {
            ...
            // Manage post content
            if (result != undefined) {
                shareURL('content', result.title, result.url);
                ...
                newState(result.title, result.url, 'content', 'content');
            }
    ...
    function newState(title, path, content, page) {
      window.history.pushState({"pageTitle":document.title, "page": $('.pt-page-current').attr('id')},"", path);
      document.title = titleCase(title) + ' - Simple Thoughts';
    }
